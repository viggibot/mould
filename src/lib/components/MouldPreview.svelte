<script>
	...
	function buildMouldGroup(d) {
		...
		const shellMat = (planes, opacity) =>
			new THREE.MeshStandardMaterial({
				color: FRAME_COLOR, metalness: 0.05, roughness: 0.85, transparent: true, opacity,
				side: THREE.DoubleSide, clippingPlanes: planes, clipShadows: false, flatShading: false
			});

		if (twoPart) {
			const planeA = new THREE.Plane(nAxis.clone(), -(splitPos + gexp));
			const planeB = new THREE.Plane(nAxis.clone().negate(), splitPos - gexp);
			const a = new THREE.Mesh(geo, shellMat([planeA], 0.55));
			a.position.copy(nAxis.clone().multiplyScalar(gexp));
			mouldGroup.add(a);
			const b = new THREE.Mesh(geo, shellMat([planeB], 0.55));
			b.position.copy(nAxis.clone().multiplyScalar(-gexp));
			mouldGroup.add(b);
		} else {
			mouldGroup.add(new THREE.Mesh(geo, shellMat([], 0.5)));
		}

		// amber silicone envelope (the pour region), open-topped, stays centred
		if (d.envelope && d.envelope.length) {
			const eg = new THREE.BufferGeometry();
			eg.setAttribute('position', new THREE.BufferAttribute(d.envelope, 3));
			eg.computeVertexNormals();
			mouldGroup.add(new THREE.Mesh(eg, new THREE.MeshBasicMaterial({ color: SILICONE_COLOR, transparent: true, opacity: 0.14, side: THREE.DoubleSide, depthWrite: false })));
		}
